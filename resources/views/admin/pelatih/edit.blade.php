...
@section('subtitle', 'Ubah data pelatih ekstrakurikuler')
        ...
        <div class="card border-0 shadow-sm rounded-4 overflow-hidden" style="background: #ffffff;">
            ...
            <div class="card-header border-0 py-4 px-5 hero-gradient">
                <div class="d-flex align-items-center gap-4">
                    ...
                    <div>
                        <h4 class="text-white fw-bold mb-0" style="font-size: 22px; letter-spacing: -0.5px;">Edit Pelatih</h4>
                        <p class="text-white-50 mb-0 small" style="font-weight: 400;">Ubah data pelatih ekstrakurikuler</p>
                    </div>
                    <div class="ms-auto">
                        <span class="badge bg-white bg-opacity-20 text-white rounded-pill px-4 py-2" style="font-weight: 500;">
                            <i class="fas fa-id-card me-2"></i>ID: #{{ str_pad($pelatih->id, 4, '0', STR_PAD_LEFT) }}
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div class="card-body p-5">
                <form action="{{ route('admin.pelatih.update', $pelatih->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="col-md-6">
                            <div class="form-group-modern">
                                ...
                                <input type="text" class="form-control form-control-modern @error('name') is-invalid @enderror" 
                                       name="name" value="{{ old('name', $pelatih->name) }}" placeholder="Masukkan nama lengkap" required>
                                @error('name')
                        ...
                        <div class="col-md-6">
                            <div class="form-group-modern">
                                ...
                                <input type="text" class="form-control form-control-modern @error('no_hp') is-invalid @enderror" 
                                       name="no_hp" value="{{ old('no_hp', $pelatih->no_hp) }}" placeholder="Masukkan nomor HP" required>
                                @error('no_hp')
                                    <div class="invalid-feedback" style="color: #dc2626; font-size: 13px;">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="col-md-6">
                            <div class="form-group-modern">
                                ...
                                <input type="email" class="form-control form-control-modern @error('email') is-invalid @enderror" 
                                       name="email" value="{{ old('email', $pelatih->email) }}" placeholder="contoh: pelatih@gmail.com" required>
                                @error('email')
                                    <div class="invalid-feedback" style="color: #dc2626; font-size: 13px;">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="form-group-modern">
                                ...
                                <select class="form-select form-select-modern @error('ekskul_id') is-invalid @enderror" 
                                    ...
                                    @foreach($ekskuls as $ekskul)
                                        <option value="{{ $ekskul->id }}" {{ old('ekskul_id', $pelatih->ekskul_id) == $ekskul->id ? 'selected' : '' }}>
                                            {{ $ekskul->nama_ekskul }}
                                        </option>
                                    @endforeach
                                </select>
