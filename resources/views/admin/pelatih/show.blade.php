@extends('layouts.app')

@section('title', 'Detail Pelatih')
@section('subtitle', 'Informasi lengkap pelatih')

@section('content')
<div class="row g-4">
    <!-- Profile Card -->
    <div class="col-xl-4 col-lg-5">
        <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
            <!-- Header Profile - Biru Cerah -->
            <div class="card-header border-0 py-4 px-4 hero-gradient">
                <div class="text-center">
                    <div class="position-relative d-inline-block">
                        @if($pelatih->avatar)
                            <img src="{{ asset($pelatih->avatar) }}"
                                 class="rounded-circle border border-3 border-white shadow-lg"
                                 style="width: 120px; height: 120px; object-fit: cover;">
                        @else
                            <div class="avatar-large bg-white bg-opacity-20 rounded-circle d-flex align-items-center justify-content-center"
                                 style="width: 120px; height: 120px; margin: 0 auto; border: 4px solid rgba(255,255,255,0.3);">
                                <i class="fas fa-chalkboard-teacher fa-4x text-white opacity-90"></i>
                            </div>
                        @endif
                        <span class="position-absolute bottom-0 end-0 bg-success rounded-circle p-2 border border-white">
                            <span class="d-block" style="width: 8px; height: 8px;"></span>
                        </span>
                    </div>
                    <h4 class="text-white fw-bold mt-3 mb-0" style="font-size: 20px; letter-spacing: -0.5px;">{{ $pelatih->name }}</h4>
                    <p class="text-white-50 small mb-0" style="font-weight: 400;">{{ $pelatih->email }}</p>
                    <div class="mt-2">
                        <span class="badge-role" style="background: rgba(255,255,255,0.12); color: #fff; padding: 4px 16px; border-radius: 20px; font-size: 12px; font-weight: 500; backdrop-filter: blur(12px); border: 1px solid rgba(255,255,255,0.06);">
                            <i class="fas fa-shield-alt me-1"></i>
                            {{ ucfirst($pelatih->role) }}
                        </span>
                    </div>
                </div>
            </div>

            <!-- Body Profile -->
            <div class="card-body p-4">
                <div class="d-flex justify-content-between mb-3 pb-3 border-bottom" style="border-color: #f1f5f9 !important;">
                    <span class="text-muted small" style="color: #64748b; font-size: 12px;">Status</span>
                    <span class="badge-status" style="background: rgba(16,185,129,0.08); color: #10b981; padding: 2px 14px; border-radius: 12px; font-size: 11px; font-weight: 500; display: inline-flex; align-items: center; gap: 6px;">
                        <span class="dot" style="width: 6px; height: 6px; background: #10b981; border-radius: 50%; display: inline-block;"></span>
                        Aktif
                    </span>
                </div>
                <div class="d-flex justify-content-between mb-3 pb-3 border-bottom" style="border-color: #f1f5f9 !important;">
                    <span class="text-muted small" style="color: #64748b; font-size: 12px;">Bergabung</span>
                    <span class="text-dark fw-semibold small" style="color: #0f172a; font-size: 13px; font-weight: 600;">
                        <i class="far fa-calendar-alt me-1 text-muted"></i>
                        {{ $pelatih->created_at->format('d M Y') }}
                    </span>
                </div>
                <div class="d-flex justify-content-between">
                    <span class="text-muted small" style="color: #64748b; font-size: 12px;">ID Pelatih</span>
                    <span class="text-dark fw-semibold small" style="color: #0f172a; font-size: 13px; font-weight: 600;">#{{ str_pad($pelatih->id, 4, '0', STR_PAD_LEFT) }}</span>
                </div>
            </div>

            <!-- Footer -->
            <div class="card-footer bg-transparent border-0 pt-0 px-4 pb-4">
                <div class="d-grid gap-2">
                    <a href="{{ route('admin.pelatih.edit', $pelatih->id) }}" class="btn-edit btn-primary-gradient" style="display: block; width: 100%; padding: 12px; font-weight: 600; font-size: 14px; text-align: center; text-decoration: none; transition: all 0.3s ease;">
                        <i class="fas fa-edit me-2"></i> Edit Pelatih
                    </a>
                    <form action="{{ route('admin.pelatih.destroy', $pelatih->id) }}" method="POST"
                          onsubmit="return confirm('Yakin ingin menghapus pelatih {{ $pelatih->name }}?')">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn-delete w-100" style="padding: 12px; border: none; border-radius: 10px; background: rgba(239,68,68,0.04); color: #ef4444; font-weight: 600; font-size: 14px; text-align: center; transition: all 0.3s ease; cursor: pointer;">
                            <i class="fas fa-trash-alt me-2"></i> Hapus Pelatih
                        </button>
                    </form>
                </div>
            </div>
        </div>
    </div>

    <!-- Info Detail -->
    <div class="col-xl-8 col-lg-7">
        <div class="card border-0 shadow-sm rounded-4">
            <div class="card-header border-0 py-3 px-4 bg-transparent">
                <h5 class="fw-bold mb-0" style="color: #0f172a; font-size: 18px;">
                    <i class="fas fa-info-circle me-2" style="color: #0ea5e9;"></i>
                    Informasi Lengkap
                </h5>
            </div>
            <div class="card-body px-4">
                <div class="row g-4">
                    <!-- Personal Info -->
                    <div class="col-12">
                        <h6 class="fw-semibold mb-3" style="color: #0ea5e9; font-size: 14px;">
                            <i class="fas fa-user me-2"></i> Data Personal
                        </h6>
                        <div class="row g-3">
                            <div class="col-md-6">
                                <div class="info-item" style="padding: 10px 14px; background: #f8fafc; border-radius: 10px; border: 1px solid rgba(0,0,0,0.02); transition: all 0.3s ease;">
                                    <label class="info-label" style="display: block; font-size: 11px; color: #64748b; font-weight: 500; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 2px;">
                                        <i class="fas fa-user-circle me-1 text-muted"></i> Nama Lengkap
                                    </label>
                                    <p class="info-value" style="font-size: 15px; font-weight: 600; color: #0f172a; margin: 0;">{{ $pelatih->name }}</p>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="info-item" style="padding: 10px 14px; background: #f8fafc; border-radius: 10px; border: 1px solid rgba(0,0,0,0.02); transition: all 0.3s ease;">
                                    <label class="info-label" style="display: block; font-size: 11px; color: #64748b; font-weight: 500; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 2px;">
                                        <i class="fas fa-envelope me-1 text-muted"></i> Email
                                    </label>
                                    <p class="info-value" style="font-size: 15px; font-weight: 600; color: #0f172a; margin: 0;">{{ $pelatih->email }}</p>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="info-item" style="padding: 10px 14px; background: #f8fafc; border-radius: 10px; border: 1px solid rgba(0,0,0,0.02); transition: all 0.3s ease;">
                                    <label class="info-label" style="display: block; font-size: 11px; color: #64748b; font-weight: 500; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 2px;">
                                        <i class="fas fa-phone me-1 text-muted"></i> No HP
                                    </label>
                                    <p class="info-value" style="font-size: 15px; font-weight: 600; color: #0f172a; margin: 0;">{{ $pelatih->no_hp ?? '-' }}</p>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="info-item" style="padding: 10px 14px; background: #f8fafc; border-radius: 10px; border: 1px solid rgba(0,0,0,0.02); transition: all 0.3s ease;">
                                    <label class="info-label" style="display: block; font-size: 11px; color: #64748b; font-weight: 500; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 2px;">
                                        <i class="fas fa-calendar-check me-1 text-muted"></i> Ekskul Binaan
                                    </label>
                                    <p class="info-value" style="font-size: 15px; font-weight: 600; color: #0f172a; margin: 0;">{{ $pelatih->ekskul->nama_ekskul ?? '-' }}</p>
                                </div>
                            </div>
                        </div>
                    </div>

                    <div class="col-12"><hr style="border-color: #f1f5f9;"></div>

                    <!-- Ekskul Info -->
                    <div class="col-12">
                        <h6 class="fw-semibold mb-3" style="color: #0ea5e9; font-size: 14px;">
                            <i class="fas fa-trophy me-2"></i> Ekstrakurikuler Binaan
                        </h6>
                        <div class="row">
                            <div class="col-12">
                                @if($pelatih->ekskul)
                                    <div class="ekskul-card" style="display: flex; align-items: center; gap: 12px; padding: 12px 18px; background: #f8fafc; border-radius: 12px; border: 1px solid rgba(0,0,0,0.02); transition: all 0.3s ease; max-width: 400px;">
                                        <div class="ekskul-icon" style="width: 40px; height: 40px; border-radius: 10px; background: rgba(14,165,233,0.06); display: flex; align-items: center; justify-content: center; color: #0ea5e9; font-size: 16px; flex-shrink: 0;">
                                            <i class="fas fa-trophy"></i>
                                        </div>
                                        <div>
                                            <div class="ekskul-name" style="font-weight: 600; font-size: 14px; color: #0f172a;">{{ $pelatih->ekskul->nama_ekskul }}</div>
                                            <div class="ekskul-meta" style="font-size: 11px; color: #64748b;">
                                                <i class="far fa-calendar-alt me-1"></i>
                                                {{ $pelatih->ekskul->hari_latihan ?? 'Hari' }}
                                                <span class="mx-1">•</span>
                                                <i class="far fa-clock me-1"></i>
                                                {{ \Carbon\Carbon::parse($pelatih->ekskul->jam_mulai)->format('H:i') }}
                                            </div>
                                        </div>
                                    </div>
                                @else
                                    <div class="text-center py-3">
                                        <i class="fas fa-inbox fa-2x text-muted mb-2 d-block" style="color: #64748b;"></i>
                                        <span class="text-muted" style="color: #64748b;">Belum ditugaskan ke ekskul manapun</span>
                                    </div>
                                @endif
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Footer -->
            <div class="card-footer border-0 bg-transparent px-4 pb-4 pt-0">
                <a href="{{ route('admin.pelatih.index') }}" class="btn-back" style="display: inline-flex; align-items: center; padding: 10px 24px; border: 1px solid #e2e8f0; border-radius: 10px; background: transparent; color: #64748b; font-weight: 500; font-size: 13px; text-decoration: none; transition: all 0.3s ease;">
                    <i class="fas fa-arrow-left me-2"></i> Kembali ke Daftar Pelatih
                </a>
            </div>
        </div>
    </div>
</div>

<style>
    .info-item:hover { background: #f1f5f9; transform: translateY(-2px); }
    .ekskul-card:hover { background: #f1f5f9; transform: translateY(-2px); box-shadow: 0 4px 16px rgba(0,0,0,0.04); }
    .btn-edit:hover { transform: translateY(-2px); box-shadow: 0 8px 30px rgba(14,165,233,0.4); color: #fff; text-decoration: none; }
    .btn-delete:hover { background: rgba(239,68,68,0.08); transform: translateY(-2px); }
    .btn-back:hover { background: #f8fafc; transform: translateY(-2px); box-shadow: 0 4px 16px rgba(0,0,0,0.04); color: #0f172a; text-decoration: none; }
    @keyframes pulse { 0%, 100% { opacity: 1; transform: scale(1); } 50% { opacity: 0.3; transform: scale(0.7); } }
    .badge-status .dot { animation: pulse 2s infinite; }
</style>
@endsection
